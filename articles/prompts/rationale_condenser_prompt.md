...
You will be provided with two or more rationales that were each generated from a subset of claims belonging to the same evaluation dimension of a research project or DeSci initiative. Each partial rationale covers a different portion of the evidence but all relate to the same dimension.
...
Retain any closing caveats about sample size, perfect scores, or complete failure that appeared in the partial rationales, but include each caveat only once. If the partial rationales between them trigger both a small-sample caveat and a perfect-score caveat for the same dimension, write a single merged closing sentence that conveys both points rather than two separate sentences. Do not repeat the same caveat idea under different wording.
